...
- `unitTest`: 19 suites, 37 tests, zero failures/errors/skips.
...
- The two Gradle test tasks cover all 173 test classes without overlap. On the
  current worktree, `parityTest --rerun-tasks` completed in 8m26s and
  `unitTest assemble --rerun-tasks` completed in 7s. The
...
| Graphical debugger | Covered | jMonkeyEngine catalog, indefinite stepping, navigation, controls, counters, diagnostic toggles, labels, and debugger-owned multithreading |
