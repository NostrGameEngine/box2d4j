...
public final class JointSeparation {
    private static final int DEFAULT_STEP_COUNT = 120;
    private static final int COUNT = 5;

    private JointSeparation() {
    }
    ...
    public static Result run(int stepCount) {
        ...
        float[] values = {500.0f, 60.0f, 2.0f};
        SampleRuntime.slider("separation.gravity", "Gravity", -10.0f, -500.0f, 500.0f, 10.0f,
            value -> b2World_SetGravity(worldId, new b2Vec2(0.0f, value)));
        SampleRuntime.action("separation.impulse", "Impulse", () -> {
            for (b2BodyId body : bodies) {
                b2Vec2 point = b2Body_GetWorldPoint(body, new b2Vec2(1.0f, 1.0f));
                b2Body_ApplyLinearImpulse(body, new b2Vec2(values[0], -values[0]), point, true);
            }
        });
        SampleRuntime.slider("separation.magnitude", "Magnitude", values[0], 0.0f, 1000.0f, 10.0f,
            value -> values[0] = value);
        SampleRuntime.slider("separation.hertz", "Hertz", values[1], 15.0f, 120.0f, 1.0f, value -> {
            values[1] = value;
            tuneJoints(joints, values[1], values[2]);
        });
        SampleRuntime.slider("separation.damping", "Damping", values[2], 0.0f, 10.0f, 0.1f, value -> {
            values[2] = value;
            tuneJoints(joints, values[1], values[2]);
        });
    ...
    private static void tuneJoints(b2JointId[] joints, float hertz, float damping) {
    ...
    private static JointState jointState(b2JointId jointId) {
        ...
        return new JointState(type, b2Joint_GetLinearSeparation(jointId), b2Joint_GetAngularSeparation(jointId),
            metricA, metricB, force.x, force.y, b2Joint_GetConstraintTorque(jointId));
    }
    ...
    public static final class Result {
        ...
        public String toLine() {
            ...
            builder.append("jointSeparation ")
    ...
    public static final class JointState {
        ...
        JointState(int type, float linearSeparation, float angularSeparation, float metricA, float metricB,
        ...
        String toLinePart() {
            return String.format(Locale.ROOT, " %d %s %s %s %s %s %s %s", type, formatFloat(linearSeparation),
                formatFloat(angularSeparation), formatFloat(metricA), formatFloat(metricB), formatFloat(forceX),
